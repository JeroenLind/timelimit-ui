...
def broadcast_event(event, data):
    """Broadcast an event to all long-poll listeners."""
...
def get_config():
    """Retrieve the current configuration from Home Assistant."""
...
class ThreadedHTTPServer(ThreadingMixIn, socketserver.TCPServer):
    daemon_threads = True
    # Enable daemon threads and address reuse for threaded HTTP server
    allow_reuse_address = True

class TimeLimitHandler(http.server.SimpleHTTPRequestHandler):
    protocol_version = "HTTP/1.1"
    def log_message(self, format, *args):
        """Log HTTP messages from the web UI."""
        log(f"WebUI: {format%args}")

    def do_POST(self):
        """Handle all API POST requests from the dashboard with detailed logging."""
